/**
 * Property-Based Tests for Room Board Component Payment Calculation
 * 
 ...
 * that the payment calculation logic in the component satisfies correctness properties.
 */
 ...
 * This mirrors the logic in room-board.component.ts totalPaidFromHistory computed signal
 */
...
/**
 * Helper function to calculate remaining deposit
 * This mirrors the logic in room-board.component.ts remainingDeposit computed signal
 */
function calculateRemainingDeposit(paymentHistory: PaymentHistory[]): number {
  const payments = paymentHistory.filter(p => p.status === PaymentStatus.SUCCESS);
...
describe('Room Board Component - Payment Calculation Property-Based Tests', () => {
  ...
  describe('Property 3: Non-negative total', () => {
    it('should always produce a valid number', () => {
      fc.assert(
        fc.property(
  ...
  /**
   * Additional tests for component-specific computed signals
   */
  describe('Component computed signals', () => {
    it('remainingDeposit should correctly calculate deposit minus refunds', () => {
      fc.assert(
        fc.property(
          fc.array(paymentHistoryArb, { minLength: 1, maxLength: 20 }),
          (paymentHistory) => {
            const result = calculateRemainingDeposit(paymentHistory);
            
            // Calculate expected manually
            const successPayments = paymentHistory.filter(p => p.status === PaymentStatus.SUCCESS);
            const depositSum = successPayments
              .filter(p => p.paymentType === PaymentType.DEPOSIT)
              .reduce((sum, p) => sum + Number(p.amount), 0);
            const refundSum = successPayments
              .filter(p => p.paymentType === PaymentType.REFUND)
              .reduce((sum, p) => sum + Number(p.amount), 0);
            const expected = depositSum - refundSum;
    ...
    it('displayedPayments should only include SUCCESS payments', () => {
      fc.assert(
        fc.property(
          fc.array(paymentHistoryArb, { minLength: 1, maxLength: 20 }),
          (paymentHistory) => {
            // Simulate displayedPayments computed signal
            const displayed = paymentHistory
              .filter(payment => payment.status === PaymentStatus.SUCCESS)
              .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
            
            // Verify all displayed payments have SUCCESS status
            return displayed.every(p => p.status === PaymentStatus.SUCCESS);
          }
        ),
        { numRuns: 100 }
      );
    ...
    it('canRefund should be true only when remainingDeposit > 0', () => {
      fc.assert(
        fc.property(
          fc.array(paymentHistoryArb, { minLength: 1, maxLength: 20 }),
          (paymentHistory) => {
            const remainingDeposit = calculateRemainingDeposit(paymentHistory);
            const canRefund = remainingDeposit > 0;
  ...
  describe('Edge cases', () => {
    ...
    it('should handle payment history with only refunds', () => {
      const paymentHistory: PaymentHistory[] = [
        {
          paymentId: '1',
          bookingId: '1',
      ...
      const result = calculateEffectivePaid(paymentHistory);
      expect(result).toBe(-800); // 0 - 500 - 300 (no deposits, only refunds)
    });
  });
});
